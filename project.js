...
const deposite = () => {
    while(true){
        const depositAmout = prompt("Enter a deposit amout: ");
        const numberDepositeAmout = parseFloat(depositAmout); 

        if(isNaN(numberDepositeAmout) || numberDepositeAmout <= 0) {
            console.log("Invalid deposit amout, try again.");
        } else {
            return numberDepositeAmout;
        }
    }
};
